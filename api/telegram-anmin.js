// Vercel Serverless Function — BudgetMein Admin Telegram Proxy
// Token & Chat ID stored in Vercel Environment Variables only
...
export default async function handler(req, res) {
  ...
  const chatId = process.env.TG_CHAT_ORDER;

  if (!token) return res.status(500).json({ ok: false, description: "Bot token not configured" });
  if (!chatId) return res.status(500).json({ ok: false, description: "Chat ID not configured" });
